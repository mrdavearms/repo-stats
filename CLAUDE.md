...
**Bulk PDF's update checks are measurable only from v2.17 (released 2026-09-22).** Before that, its `check_for_update()` in `pdf_generator.py` sent a HEAD request to `github.com/.../releases/latest` (a redirect), deliberately avoiding api.github.com because a school shares one IP and would hit the 60/hour unauthenticated limit. GitHub counts that request nowhere: not in asset downloads, not in traffic views/paths (verified 2026-09-22: `/releases/latest` never appeared in its popular paths). From v2.17 the app first GETs `latest.json` via `.../releases/latest/download/latest.json` (attached by its `release.yml`), which GitHub counts as a download of that asset, and falls back to the redirect only if the manifest is missing. Consequences: (1) `has_updater` / `hasUpdater()` flips true once the collector sees v2.17's `latest.json`; before that the email and dashboard say "not measurable", never 0; (2) copies older than v2.17 stay invisible until they update, so the figure starts near zero and grows; (3) the count only ever covers the *latest* release's manifest being fetched via the `latest/download` link, which is the intended behaviour.
